...
Windows 本地 wasm 单独运行 100/1,000/10,000 条容量用例为 3/3 通过；2026-09-26 一次测量墙钟约 559 ms，包含测试进程启动开销。Ubuntu CI run `36245555879` 对同三组用例的 `/usr/bin/time -v` 记录为 0.34 s、最大常驻内存 67,120 KB，退出码 0。这些数字只描述当次机器与工具链，不是性能承诺；版本冻结后仍需在冻结提交上重新取证。
